...
| Sujet du fil | Figé au premier message, envois suivants en `Re: …`. Fil booking ouvert au premier devis, sujet généré depuis l'événement — format acté le 08/07 : « Votre événement au {restaurant} » (`buildThreadSubject`, contraction française Le/La/Les/L'), appliqué aux DEUX transports (Resend compris, dès le déploiement). Fil contact ouvert au premier ponctuel (sujet saisi dans le composer). |
