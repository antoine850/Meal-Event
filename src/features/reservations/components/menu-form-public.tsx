...
export function MenuFormPublic({ token }: Props) {
  ...
  const updateQuantity = useCallback(
    (fieldId: string, option: string, delta: number, maxTotal?: number) => {
      setQuantities((prev) => {
        ...
        if (maxTotal !== undefined && delta > 0) {
          const currentTotal = Object.values(fieldQty).reduce(
            (sum, qty) => sum + qty,
            0
          )
  ...
  const setQuantityDirect = useCallback(
    (fieldId: string, option: string, value: string, maxTotal?: number) => {
  ...
  const getFieldTotal = useCallback(
    (fieldId: string) => {
        ...
        <Card className='mx-4 w-full max-w-md'>
          <CardContent className='space-y-2 pt-6 text-center'>
            ...
            <p className='text-sm text-muted-foreground'>
              Ce lien est invalide ou le formulaire n'est plus disponible.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  const fields = (menuForm?.menu_form_fields || []).sort(
    ...
    for (const field of fields) {
      if (field.field_type === 'select') {
        ...
        if (field.is_required && actualTotal === 0) {
          errors[field.id] =
            `Ce champ est requis. Veuillez sélectionner au moins une option.`
        } else if (field.is_per_person && actualTotal !== expectedCount) {
          errors[field.id] =
            `Le total doit être exactement ${expectedCount} (actuellement ${actualTotal})`
        }
      }
    }
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
      ...
      {restaurant && (
        <div
          className='w-full px-4 py-8 text-white shadow-lg'
        ...
        >
          <div className='mx-auto max-w-4xl'>
            <div className='flex items-start justify-between gap-6'>
                  ...
                  <div className='space-y-1 text-sm text-white/90'>
                    {restaurant.address && (
                      <div className='flex items-center gap-2'>
                        ...
                        <span>
                          {restaurant.address}, {restaurant.postal_code}{' '}
                          {restaurant.city}
                        </span>
                      </div>
                    )}
              ...
              {/* Right: Booking Info */}
              {booking && (
                <div className='min-w-[280px] rounded-lg bg-white/10 p-4'>
                  ...
                  <div className='space-y-2 text-sm'>
                    ...
                    {booking.guests_count && (
                      <div className='flex items-center gap-2'>
                        ...
                        <span>
                          {booking.guests_count} convive
                          {booking.guests_count > 1 ? 's' : ''}
                        </span>
                      </div>
                    )}
                    {booking.occasion && (
                      <div className='mt-2 border-t border-white/20 pt-2 text-xs text-white/70'>
                        {booking.occasion}
                      </div>
                    )}
                  </div>
            ...
            <CardHeader>
              <div className='space-y-2'>
                <CardTitle className='text-xl'>
                  {menuForm?.title || 'Formulaire de menu'}
                </CardTitle>
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <div>
                    <p className='text-sm font-semibold text-green-800'>
                      Formulaire soumis et verrouillé
                    </p>
                    <p className='text-xs text-green-600'>
                      Vos choix ont été enregistrés. Contactez l'organisateur si
                      vous souhaitez les modifier.
                    </p>
                  </div>
                </div>
              </CardContent>
                  ...
                  <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-2'>
                      ...
                      {field.is_required && (
                        <Badge
                          variant='outline'
                          className='border-red-200 text-[9px] text-red-500'
                        >
                          Requis
                        </Badge>
                      )}
                      {field.is_per_person && (
                        <Badge variant='outline' className='gap-0.5 text-[9px]'>
                          <Users className='h-2.5 w-2.5' /> {expectedCount}{' '}
                          convive{expectedCount > 1 ? 's' : ''}
                        </Badge>
                      )}
                    </div>
                        ...
                        >
                          ...
                          <div className='flex items-center gap-2'>
                            <Button
                              ...
                              disabled={
                                isSubmitted || (fieldQty[opt.label] || 0) === 0
                              }
                            >
                            ...
                            <Input
                              ...
                              className='h-8 w-14 [appearance:textfield] p-0 text-center text-sm [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none'
                            />
                            <Button
                              size='sm'
                              variant='outline'
                              ...
                              disabled={
                                isSubmitted || currentTotal >= expectedCount
                              }
                            >
                      ...
                      <div className='flex items-center justify-between rounded-lg border bg-muted/30 p-3'>
                        ...
                        <div className='flex items-center gap-2'>
                          <Button
                            ...
                            disabled={
                              isSubmitted || (fieldQty['__empty__'] || 0) === 0
                            }
                          >
                            <Minus className='h-3 w-3' />
                          </Button>
