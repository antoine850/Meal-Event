...
export function getBookingRefDate(
  b: BookingWithRelations,
  field: DashboardDateField
): Date | null {
  if (field === 'event_date')
    return b.event_date ? parseISO(b.event_date) : null
  if (field === 'created_at')
    return b.created_at ? parseISO(b.created_at) : null
  if (field === 'signed_at') {
    const primary =
      b.quotes?.find((q) => q.primary_quote) ||
      b.quotes?.find((q) => q.quote_signed_at)
  ...
  const { data: currentUser } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: dbUser } = await supabase
        ...
        .single()
      return {
        firstName: dbUser?.first_name || authUser.email?.split('@')[0] || '',
...
export function calcSignedCount(bookings: BookingWithRelations[]) {
  return bookings.filter((b) => SIGNED_SLUGS.includes(b.status?.slug || ''))
    .length
}
...
export function calcPipeline(
  ...
  statuses.forEach((s) => {
    ...
    if (statusBookings.length > 0 || true) {
      // show all statuses
      pipeline.push({
        statusId: s.id,
        name: s.name,
        ...
        amount: statusBookings.reduce(
          (sum, b) => sum + getPipelineQuoteTtc(b),
          0
        ),
      })
    }
  ...
  bookings.forEach((b) => {
    ...
    if (ids.length === 0) {
      if (!groups['unassigned'])
        groups['unassigned'] = { user: null, bookings: [] }
...
export function getMonthlyRevenueByRestaurant(
  bookings: BookingWithRelations[]
) {
  // Get last 6 months
  const now = new Date()
  ...
  const restaurantNames = [
    ...new Set(bookings.map((b) => b.restaurant?.name).filter(Boolean)),
  ] as string[]
...
export function getMonthlyRevenueByCommercial(
  bookings: BookingWithRelations[],
  users: { id: string; first_name: string; last_name: string }[]
) {
    ...
    commercialNames.forEach((name) => {
      row[name] = monthBookings
        .filter((b) => {
          const ids = b.assigned_user_ids || []
          return (
            ids.some((id) => userMap.get(id) === name) &&
            SIGNED_SLUGS.includes(b.status?.slug || '')
          )
        })
        .reduce((sum, b) => sum + getSignedQuoteTtc(b), 0)
...
export function getContactsBySource(
  contacts: ContactWithRelations[],
  bookings: BookingWithRelations[]
) {
  const sourceGroups: Record<
    string,
    { leads: number; bookings: number; signed: number; revenue: number }
  > = {}

  contacts.forEach((c) => {
    const source = (c as any).source || 'Autre'
    if (!sourceGroups[source])
      sourceGroups[source] = { leads: 0, bookings: 0, signed: 0, revenue: 0 }
  ...
  return Object.entries(sourceGroups)
    .map(([source, data]) => ({
      ...
      conversionRate:
        data.leads > 0
          ? Math.round((data.bookings / data.leads) * 1000) / 10
          : 0,
      /** Taux de signature = signés / leads (pour identifier la "meilleure source") */
      signatureRate:
        data.leads > 0 ? Math.round((data.signed / data.leads) * 1000) / 10 : 0,
...
export function getMonthlyLeadsBySource(contacts: ContactWithRelations[]) {
  ...
  const sources = [
    ...new Set(contacts.map((c) => (c as any).source || 'Autre')),
  ]
  ...
  return months.map((m) => {
    ...
    sources.forEach((source) => {
      row[source] = monthContacts.filter(
        (c) => ((c as any).source || 'Autre') === source
      ).length
    })
    return row
  })
