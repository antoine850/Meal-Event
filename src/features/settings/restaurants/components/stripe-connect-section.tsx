...
export function StripeConnectSection({ restaurant }: Props) {
  ...
  const handleConnect = () => {
    startConnect.mutate(restaurant.id, {
      onError: (err: Error) =>
        toast.error(
          err instanceof Error ? err.message : 'Erreur de connexion Stripe'
        ),
    })
  }
        ...
        {!isConnected && (
          <>
            <p className='text-sm text-muted-foreground'>
              Aucun compte Stripe connecté. Les paiements utilisent le virement
              bancaire ou la clé plateforme.
            </p>
            {isAdmin ? (
          ...
          <>
            <div className='flex flex-col gap-2'>
              <p className='text-sm font-medium'>
                {restaurant.stripe_account_name || restaurant.stripe_account_id}
              </p>
              ...
              {restaurant.stripe_connected_at && (
                <p className='text-xs text-muted-foreground'>
                  Connecté le{' '}
                  {new Date(restaurant.stripe_connected_at).toLocaleDateString(
                    'fr-FR'
                  )}
                </p>
              )}
              <div className='flex flex-wrap gap-2'>
                ...
                <Badge
                  variant={
                    restaurant.stripe_payouts_enabled ? 'default' : 'secondary'
                  }
                  className='gap-1'
                >
