...
export function verifyWebhookSignature(payload: string | Buffer, signature: string): boolean {
  ...
  // SignNow HMAC: SHA256 hex → hex2bin → base64
  // Ref: https://www.signnow.com/blog/hmac-security/
  const body = typeof payload === 'string' ? payload : payload.toString('utf8')
  const hexHash = crypto.createHmac('sha256', Buffer.from(secret, 'utf8')).update(body).digest('hex')
  ...
  if (sigBuffer.length !== expectedBuffer.length) {
    console.warn(`[SignNow] Signature length mismatch: received "${signature}" (${sigBuffer.length}), expected (${expectedBuffer.length})`)
    return false
  }
