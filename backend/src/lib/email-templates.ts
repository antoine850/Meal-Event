...
// Format des montants envoyés au client : entier si rond, sinon 2 décimales
// (aligné sur le montant réellement facturé côté Stripe).
function formatCurrency(amount: number): string {
  return formatEuroAdaptive(amount)
}
...
function buildLegalFooter(r: RestaurantBranding): string {
  ...
  if (r.postal_code || r.city)
    parts.push(`${r.postal_code || ''} ${r.city || ''}`.trim())
...
export function buildDepositEmailHtml(params: {
  ...
  const stripeCta =
    stripeEnabled && stripePaymentUrl
      ? `
    <!-- CTA Button -->
...
export function buildBalanceEmailSubject(
  quoteNumber: string,
  restaurantName: string
): string {
...
export function buildPaymentLinkEmailHtml(params: {
  ...
  const virementLabel =
    modality === 'acompte'
      ? `ACOMPTE${quoteNumber ? '-' + quoteNumber : ''}`
      : modality === 'solde'
        ? `SOLDE${quoteNumber ? '-' + quoteNumber : ''}`
        : `PAIEMENT${quoteNumber ? '-' + quoteNumber : ''}`
...
export function buildPaymentLinkEmailSubject(
  modality: 'acompte' | 'solde' | 'autre',
  restaurantName: string,
  quoteNumber: string | null
): string {
...
export function buildSignatureNotificationSubject(
  quoteNumber: string,
  contactName: string
): string {
...
export function buildPaymentNotificationHtml(params: {
  ...
  const typeLabel =
    paymentType === 'deposit' || paymentType === 'acompte'
      ? 'Acompte'
      : paymentType === 'balance' || paymentType === 'solde'
        ? 'Solde'
        : paymentType === 'extra'
          ? 'Extra'
          : 'Paiement'
...
export function buildPaymentNotificationSubject(
  amount: number,
  contactName: string,
  paymentType: string
): string {
  const typeLabel =
    paymentType === 'deposit' || paymentType === 'acompte'
      ...
      : paymentType === 'balance' || paymentType === 'solde'
        ? 'Solde'
        : 'Paiement'
  return `${typeLabel} de ${formatEuroAdaptive(amount)} reçu — ${contactName}`
}
