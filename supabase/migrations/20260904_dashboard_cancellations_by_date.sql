-- Carte motifs d'annulation : compter par date d'annulation (status_changed_at)
-- et non par date d'evenement. L'annulation auto des dossiers a date passee
-- (04/09, status-promotion.ts) ecrit 641 annulations le meme jour sur des events
-- de 2024-2026 : par event_date la carte les etale sur les mois passes.
-- Fonction rejouee a l'identique par ailleurs (source : 20260823_dashboard_cancellation_reasons.sql).
...
begin
  return (
    ...
    select jsonb_build_object(
      ...
      -- La periode du dashboard (p_from_event / p_to_event) s'applique ici a la
      -- date d'annulation, en jour Paris : l'annulation auto tourne juste apres
      -- minuit Paris, soit la veille en UTC. Les autres filtres restent ceux du
      -- scope courant.
      'by_cancellation_reason', (
        select coalesce(jsonb_agg(jsonb_build_object('name', r, 'value', cnt)
          order by cnt desc), '[]'::jsonb)
        from (
          select coalesce(b.cancellation_reason, '') r, count(*) cnt
          from public.bookings b
          join public._dashboard_booking_ids(
            null, null, p_from_sign, p_to_sign,
            p_from_import, p_to_import, p_restaurants, p_statuses, p_commercials, p_client_type) ids on ids.id = b.id
          join public.statuses st on st.id = b.status_id and st.slug = 'cancelled'
          where (p_from_event is null or (b.status_changed_at at time zone 'Europe/Paris')::date >= p_from_event)
            and (p_to_event   is null or (b.status_changed_at at time zone 'Europe/Paris')::date <= p_to_event)
          group by coalesce(b.cancellation_reason, '')
        ) c),
